# ADR-001：保持 handler-tls 为独立 Gnalloy 模块
...
`handler-tls` 作为 `gnalloy.org/handler-tls` 发布，并拥有该职责：基于 crypto/tls 的 Gnalloy TLS Pipeline Handler，覆盖 ALPN、SNI、StartTLS、Optional TLS、OCSP 与 cipher suite 辅助能力。
